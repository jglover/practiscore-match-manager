...
Stage.prototype.getStagePoints = function () {
  return this.stage_tppoints;
};
Stage.prototype.setStagePoints = function (newStagePoints) {
  this.stage_tppoints = parseInt (newStagePoints || 0);
...
Stage.prototype.getAsPlainObject = function () {
  var self = this;
  var s = stage.Stage.prototype.getAsPlainObject.call (self);

  if (self.stage_maxStringTime)
    s.stage_maxStringTime = self.stage_maxStringTime;
  if (self.stage_tppoints)
    s.stage_tppoints = self.stage_tppoints;
...
var Matchdef = function (accessorFunctions, options, callback) {
  ...
  matchdef.Matchdef.call (self, accessorFunctions, options, function (err) {
    if (!err) {
      ...
      self.match_maxstagetime = 0;
      self.match_usemaxstagetime = false;

      self.setMaxStageTime (0);
      self.setUseMaxStageTime (false);

      matchdef.Matchdef.prototype.setMatchType.call (self, 'timepluspoints');
          ...
          matchdef.Matchdef.prototype.loadDivisions.call (self, defaultValues.divisions, function (err, divisions) {
            if (!err) {
              ...
              _.each (divisions, function (division) {
                ...
                lookups.divisions_flipped [division.pmm_name] = division.ps_name;
              });
            }
            callback (err);
          });
...
Matchdef.prototype.merge = function (newMatchdef, options, callback) {
  ...
  assert (newMatchdef, 'tpp.Matchdef.prototype.merge(): newMatchdef cannot be null or undefined');
...
Matchdef.prototype.getAsPlainObject = function () {
  var self = this;
  var m = matchdef.Matchdef.prototype.getAsPlainObject.call (self);

  if (self.match_bonuses.length) {
    m.match_bonuses = [];

    _.each (self.match_bonuses, function (bonus) {
      m.match_bonuses.push (bonus.getAsPlainObject ());
    });
  }

  if (self.match_penalties.length) {
    m.match_penalties = [];

    _.each (self.match_penalties, function (penalty) {
      m.match_penalties.push (penalty.getAsPlainObject ());
    });
  }
...
Score.prototype.setBonuses = function (bonuses) {
  assert (_.isArray (bonuses), "tpp.Score.setBonuses parameter must be array");
  this.bons = bonuses || [];
...
Score.prototype.setPenalties = function (penalties) {
  assert (_.isArray (penalties), "tpp.Score.setPenalties parameter must be array");
  this.pens = penalties || [];
  this.mod  = psutils.timeStampUTC ();
  return this;
};
...
Score.prototype.getAsPlainObject = function () {
  var self = this;
  var s = score.Score.prototype.getAsPlainObject.call (self);

  if (self.bons.length) s.bons = self.bons;
  if (self.pens.length) s.pens = self.pens;
