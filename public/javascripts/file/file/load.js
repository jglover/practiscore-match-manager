...
$(function () {
  ...
  $('#confirmLoadWindow').on ('close', function (event) {
    if ((event.type === 'close') && (event.args.dialogResult.OK)) {
      ...
      App.socket.emit ('file:psc:load', {file: App._load.match.match_file}, function (data) {
        if (data.err)
  ...
  $('#confirmDeleteWindow').on ('close', function (event) {
    if ((event.type === 'close') && (event.args.dialogResult.OK)) {
      App.socket.emit ('file:psc:delete', {file: App._delete.match.match_file}, function (data) {
        if (data.err)
          $('#msgDiv').text ('System Error: ' + data.err).show ();
        else {
...
//    { text: 'UUID',          datafield:  'match_uuid',       cellsalign: 'center', width: 150 },
    ],
  ...
  App.loadFile = function (match) {
    if (App._saveCurrent) {
      ...
      App.socket.emit ('file:psc:load', {file: match.match_file}, function (data) {
        if (data.err)
          $('#msgDiv').text ('System Error: ' + data.err).show ();
        else {
          $('#msgDiv').html ('Match successfully loaded').show ();
  ...
  App.listFiles = function () {
    App.socket.emit ('file:psc:list', function (data) {
      if (data.err)
  ...
  App.socket.emit ('log:log', {'msg': 'File->File->Load Match'});
});
