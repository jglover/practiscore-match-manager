...
$(function () {
  ...
  $(grid).on ('rowdoubleclick', function (event) {
    App.combineFile ($(grid).jqxGrid ('getrowdata', event.args.rowindex));
  });
  ...
  $('#contextMenu').on ('itemclick', function (event) {
    ...
    switch ($.trim ($(event.args).text ())) {
      case 'Combine this file' :
        App.combineFile (match);
        break;
      default :
        break;
    }
  ...
  $('#confirmCombineWindow').jqxWindow ({
    height: 154,
    width: 280,
    resizable: false,
    autoOpen: false,
    ...
    okButton: $('#buttonCombineOK'),
    cancelButton: $('#buttonCombineCancel'),
    initContent: function () {
      $('#buttonCombineOK').jqxButton ({width: 65});
      $('#buttonCombineCancel').jqxButton ({width: 65});
      $('#buttonCombineCancel').focus();
    },
  });

  $('#confirmCombineWindow').on ('close', function (event) {
    if ((event.type === 'close') && (event.args.dialogResult.OK)) {
      $('#msgDiv').html ('Combining match...').show ();
      $(grid).jqxGrid ('showloadelement');
      App.socket.emit ('file:psc:combine', {file: App._combine.match.match_file}, function (data) {
        if (data.err)
          $('#msgDiv').text ('System Error: ' + data.err).show ();
        else {
          $('#msgDiv').html ('Match successfully combined').show ();
  ...
  App.combineFile = function (match) {
    if (App._saveCurrent) {
      App._combine = {match: match};
      $('#confirmCombineWindow').jqxWindow ('open');
    } else {
      $('#msgDiv').html ('Combining match...').show ();
      $(grid).jqxGrid ('showloadelement');

      App.socket.emit ('file:psc:combine', {file: match.match_file}, function (data) {
        if (data.err)
          $('#msgDiv').text ('System Error: ' + data.err).show ();
        else {
          $('#msgDiv').html ('Match successfully combined').show ();
  ...
  App.listFiles = function () {
    App.socket.emit ('file:psc:list', {matchType: true}, function (data) {
      if (data.err)
  ...
  App.socket.emit ('log:log', {'msg': 'File->File->Combine Match'});
});
