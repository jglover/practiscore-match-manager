...
$(function () {
  ...
  $(grid).on ('rowdoubleclick', function (event) {
    App.loadFile ($(grid).jqxGrid ('getrowdata', event.args.rowindex));
  });
  ...
  $('#contextMenu').on ('itemclick', function (event) {
    ...
    switch ($.trim ($(event.args).text ())) {
      ...
      case 'Delete this match' :
        App._delete = {rowId: rowId, match: match};
  ...
  $('#confirmLoadWindow').on ('close', function (event) {
    if ((event.type === 'close') && (event.args.dialogResult.OK)) {
      ...
      App.socket.emit ('file:db:load', {uuid: App._load.match.match_uuid}, function (data) {
        if (data.err)
  ...
  $('#confirmDeleteWindow').on ('close', function (event) {
    if ((event.type === 'close') && (event.args.dialogResult.OK)) {
      App.socket.emit ('file:db:delete', {uuid: App._delete.match.match_uuid}, function (data) {
        if (data.err)
          $('#msgDiv').text ('System Error: ' + data.err).show ();
        else {
          $('#msgDiv').text (App._delete.match.match_name + ' deleted').show ();
          $('#matchname').text (data.matchName);
          $(grid).jqxGrid ('deleterow', App._delete.rowId);
        }
        delete App._delete;
      });
    }
  ...
  App.loadFile = function (match) {
    if (App._saveCurrent) {
      App._load = {match: match};
      $('#confirmLoadWindow').jqxWindow ('open');
    } else {
      $('#msgDiv').html ('Loading match...').show ();
      $(grid).jqxGrid ('showloadelement');

      App.socket.emit ('file:db:load', {uuid: match.match_uuid}, function (data) {
        if (data.err)
          $('#msgDiv').text ('System Error: ' + data.err).show ();
        else {
          $('#msgDiv').html ('Match successfully loaded').show ();
  ...
  App.listFiles = function () {
    App.socket.emit ('file:db:list', function (data) {
      if (data.err)
  ...
  App.socket.on ('db_directory_changed', App.listFiles);
  App.socket.emit ('log:log', {'msg': 'File->Database->Load Match'});
});
