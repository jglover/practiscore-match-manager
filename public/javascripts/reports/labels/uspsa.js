...
$(function () {
      ...
      format: {
        competitor: function (shooter, stageName, stageNumber) {
          return [
            ...
            { html: shooter.sh_dpf,            attrs: {x:  '95.0pt', y: '48.0pt', 'text-anchor': 'middle', style: 'font-weight: bold'  }},
            { html: stageName,                 attrs: {x:  '95.0pt', y: '65.0pt', 'text-anchor': 'middle', style: 'font-size: smaller' }},
          ];
        },
        chrono: function (shooter) {
          return [
            { html: 'Chrono',                  attrs: {x:   '7.2pt', y: '14.4pt', 'text-anchor': 'start',  style: 'font-size: smaller' }},
            { html: 'Comp #' + shooter.sh_pos, attrs: {x:  '95.0pt', y: '14.4pt', 'text-anchor': 'middle', style: 'font-size: smaller' }},
            { html: 'Squad ' + shooter.sh_sqd, attrs: {x: '182.0pt', y: '14.4pt', 'text-anchor': 'end',    style: 'font-size: smaller' }},
            { html: shooter.sh_fnln,           attrs: {x:  '95.0pt', y: '31.0pt', 'text-anchor': 'middle', style: 'font-weight: bold'  }},
            { html: shooter.sh_dpf,            attrs: {x:  '95.0pt', y: '48.0pt', 'text-anchor': 'middle', style: 'font-weight: bold'  }},
            { html: 'Time: _____',             attrs: {x:   '7.2pt', y: '65.0pt', 'text-anchor': 'start',  style: 'font-size: smaller' }},
            { html: 'Bullet Weight: ____',     attrs: {x: '182.0pt', y: '65.0pt', 'text-anchor': 'end',    style: 'font-size: smaller' }},
          ];
        },
        verification: function (shooter) {
          ...
          line.push (shooter.sh_dvp + ' (' + shooter.sh_grd + ') ' + shooter.sh_pf + ' (' + shooter.sh_sqd + ') (' + shooter.sh_st + ')');
  ...
  App.renderLabels = function (pv) {
    ...
    pmelib.labelEngine (pv, printList,
      function (shooter, addLabelCallback) {
        ...
        _.times (pv.numberOfChronoLabels, function () {
          addLabelCallback (pv.labelSpec.format.chrono (shooter));
        });
  ...
  App.updatePrintVariables = function () {
    ...
    var printChrono       = $('#checkbox-labels-enabled-chrono').jqxCheckBox ('checked');
    ...
    pv.numberOfChronoLabels       = printChrono       ? $('#numberinput-labels-copies-chrono').jqxNumberInput ('val')       : 0;
    ...
    pv.clearQueue = $('#radiobutton-labels-print-queued').jqxRadioButton ('checked') &&
                    $('#checkbox-labels-print-clearqueue').jqxCheckBox ('checked');
  ...
  App.updateButtonStates = function () {
    ...
    var chronoLabelsEnabled = $('#checkbox-labels-enabled-chrono').jqxCheckBox ('checked');
    ...
    $('[no-chrono-labels-hide]').toggle (chronoLabelsEnabled);
  ...
  if (window.matchMedia) {
    ...
    mediaQueryList.addListener (function (mql) {
      App.printWindowOpen = mql.matches ? true : false;
    });
  }
  ...
  $('#button-print-preview').click (function () {
    if (!App.matchDataChanged) {
      ...
      if (pv.clearQueue)
        App.socket.emit ('shooter:print:clear');
    } else {
      alert ('Match data has changed! Reload this page, please.');
  ...
  App.getMatch = function () {
    App.socket.emit ('match:get', {options: {match: true, stages: true, lookups: true}}, function (data) {
      ...
      _.each (App.matchData.m.match_shooters, function (shooter) {
        ...
        shooter.sh_grd  = App.matchData.l.classes [shooter.sh_grd]     || 'U';
        shooter.sh_dvp  = App.matchData.l.divisions [shooter.sh_dvp]   || 'Open';
        shooter.sh_pf   = App.matchData.l.powerfactors [shooter.sh_pf] || 'Minor';

        shooter.sh_fnln = shooter.sh_fn + ' ' + shooter.sh_ln;
        shooter.sh_lnfn = shooter.sh_ln + ', ' + shooter.sh_fn;
        shooter.sh_dpf  = shooter.sh_dvp + ((shooter.sh_dvp === 'Production') ? '' : (' / ' + shooter.sh_pf));
