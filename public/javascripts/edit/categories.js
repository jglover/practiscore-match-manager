...
$(function () {
  ...
  var categoriesSource = {
    ...
    addrow: function (rowid, newdata, position, commit) {
      var temp = [];
      temp.push ({
        ps_name: newdata.pmm_name,
        pmm_name: newdata.pmm_name,
        enabled: true,
        readonly: false,
      });
      App.socket.emit ('categories:add', {category: temp}, function (data) {
        if (data.err) {
          ...
          commit (false);
          categoriesSource.localdata = data.categories;
          $(grid).jqxGrid ({source: categoriesAdapter});
        }
      });
    },
    updaterow: function (rowid, newdata, commit) {
      var temp = [];
      newdata.ps_name = newdata.pmm_name;
      temp.push (newdata);
      App.socket.emit ('categories:modify', {category: temp}, function (data) {
        if (data.err) {
          editVars.errorDisplayed = true;
          _.each (data.err, function (e) {
            $(grid).jqxGrid ('showvalidationpopup', rowid, e.field, e.errmsg);
    ...
    deleterow: function (rowid, commit) {
      ...
      App.socket.emit ('categories:delete', {category: temp}, function (data) {
        if (data.err)
          commit (false);
        else {
          commit (false);
  ...
  categoriesAdapter = new $.jqx.dataAdapter (categoriesSource);
  ...
  var openPopupEditorAdd = function () {
    ...
    $(popupEdit).jqxWindow ('title', 'Add Category');
  ...
  var openPopupEditorEdit = function (rowIndex) {
    ...
    $(popupEdit).jqxWindow ('title', 'Editing Category #' + (rowIndex + 1));
    $(popupEdit).jqxWindow ('open');
  };
  ...
  var gridColumns = [
      {
        ...
        validation:      function (cell, value) {
                           if (!value.length)
                             return ({result: false, message: 'Category name is required'});
                           if (value.search (/[^\x20-\x7f]/) !== -1)
                             return ({result: false, message: 'Category name may only contain printable characters'});
                           return true;
                         },
      }, {
        text:            'Enabled',
  ...
  $(grid).jqxGrid ({
    ...
    rendered: function () {
      ...
      gridCells.on ('dragEnd', function (event) {
        ...
        if (!_.isNaN (dropRow)) {
          App.socket.emit ('categories:move', {from: dragRowData, to: dropRowData}, function (data) {
            if (data.changed) {
              categoriesSource.localdata = data.categories;
              $(grid).jqxGrid ({source: categoriesAdapter});
            }
  ...
  $(grid).on ('cellclick', function (event) {
    if (event.args.rightclick && !$(grid).find ('.jqx-grid-validation').length) {
      ...
      $('#contextEditCategoryNum').text (rowindex + 1);
      $('#contextAddCategoryNum').text ($(grid).jqxGrid ('getdatainformation').rowscount + 1);
      $('#contextDeleteCategoryNum').text (rowindex + 1);
      var scrollTop = $(window).scrollTop ();
      var scrollLeft = $(window).scrollLeft ();
      var contextMenu = App.contextMenuInit ();
      $(contextMenu).jqxMenu ('open', parseInt (event.args.originalEvent.clientX) + 5 + scrollLeft, parseInt (event.args.originalEvent.clientY) + 5 + scrollTop);
      $(contextMenu).jqxMenu ('disable', 'contextEditCategory', readonly);
      $(contextMenu).jqxMenu ('disable', 'contextDeleteCategory', readonly);
      return false;
    }
  });
  ...
  App.contextMenuInit = function () {
    ...
    $(contextMenu).on ('itemclick', function (event) {
      ...
      switch (event.args.id) {
        case 'contextEditCategory' :
          openPopupEditorEdit (selectedrowindex);
          break;

        case 'contextAddCategory' :
          openPopupEditorAdd ();
          break;

        case 'contextDeleteCategory' :
      ...
      rules: [
        {
          input:    '#ec-pmmname',
          position: 'bottom',
          message:  'PMM category name is required',
          action:   'keyup, blur',
          rule:     'required'
        }, {
          input:    '#ec-pmmname',
          position: 'bottom',
          message:  'Category name may only contain printable characters',
          action:   'keyup, blur',
          rule:     function (input) {
                      return (input.val ().search (/[^\x20-\x7f]/) === -1) ? true : false;
                    }
        },
      ]
    });
  ...
  App.refreshMatchData = function () {
    App.socket.emit ('categories:get', function (data) {
      categoriesSource.localdata = [];
      _.each (data.categories, function (category) {
        categoriesSource.localdata.push ({
          ps_name:  category.ps_name,
          pmm_name: category.pmm_name,
