...
$(function () {
    ...
    columns: [
      ...
      },
      {
        ...
        validation:      function (cell, value) {
                           var regex = /[^A-Za-z0-9\. -]/;
                           if (regex.test (value))
                             return ({result: false, message: 'Member # name may only contain the characters A-Z, a-z, 0-9, period, dash, and space'});
                           return true;
                         },
  ...
  App.contextMenuInit = function () {
    ...
    _.each (['sh_pos','sh_ln','sh_fn','sh_id','sh_dvp_fmt','sh_grd','sh_sqd','sh_ctgs_fmt','sh_eml','sh_ph','sh_st','sh_wlk_fmt','sh_dq_fmt','sh_del_fmt','sh_print_fmt'], function (n) {
      var sc = $('#sc_' + n);
  ...
  App.popupEditInit = function () {
    ...
    $('#ec-sh-id').jqxInput ({width: 105, height: 25, maxLength: 9});
    ...
    $(popupEdit).jqxValidator ({
      ...
      rules: [
        ...
        },
        {
          input:    '#ec-sh-id',
          position: 'bottom',
          message:  'This member # is in use by a competitor with a different name',
          action:   'valuechanged, blur',
          rule:     function (input) {
  ...
  App.socket.emit ('log:log', {msg: 'Edit/View->Competitors (TP)'});
