...
$(function () {
  ...
  var shooterSource = {
    ...
    updaterow: function (rowid, newdata, commit) {
      ...
      if (((newdata.sh_dvp === 'PROD') || (newdata.sh_dvp === 'CO')) && (newdata.sh_pf !== 'MINOR'))
        newdata.sh_pf = 'MINOR';
  ...
  var cellsrendererFunc = function (row, columnField, value, defaultHTML, columnProperties, shooter, remap) {
    ...
    if (shooter) {
      if ((shooter.sh_dvp === 'CO') && (columnField === 'sh_pf'))
        html1 += ' competitorCarryOptics';
      else if ((shooter.sh_dvp === 'PROD') && (columnField === 'sh_pf'))
        html1 += ' competitorProduction';
  ...
  var fillPopupFields = function (dataRecord) {
    ...
    var uspsaPrefix = dataRecord.sh_id.match (uspsaNumberRE);
    var uspsaNumber;
    ...
    $('#ec-sh-id-prefix').jqxDropDownList ('selectIndex', _.findIndex (prefixes, 'value', uspsaPrefix));
    $('#ec-sh-id-num').jqxInput ('val', uspsaNumber);

    $('#ec-sh-dvp').val (dataRecord.sh_dvp);
    $('#ec-sh-pf').val (dataRecord.sh_pf);
    ...
    $('#ec-sh-num').val (dataRecord.sh_num);
  ...
  var openPopupEditorAdd = function () {
    ...
    App.socket.emit ('shooter:maxnameslength', function (data) {
      ...
      App.socket.emit ('shooter:new', function (data) {
        ...
        shooter.sh_pf_fmt  =   _.result (_.find (powerfactors, 'value', shooter.sh_pf), 'label');
  ...
  //
  //  4.1.0 broke the look by changing the following values:
  //    columnsHeight 26 -> 31
  //    pagerHeight   29 -> 35
  //    rowsHeight    25 -> 28
  //
  $(grid).jqxGrid ({
    ...
    columns: [
      ...
      },
      {
        ...
        validation:      function (cell, value) {
                           ...
                           if (!text.length)
                             return true;
                           if (!regex.test (text))
                             return ({result: false, message: 'Invalid USPSA number'});
                           ...
                           if (count)
                             return ({result: false, message: 'This USPSA # is in use by a competitor with a different name'});
      ...
      },
      {
        text:            'PF',
        datafield:       'sh_pf',
        displayfield:    'sh_pf_fmt',
        width:           45,
        cellsrenderer:   cellsrendererFunc,
        columntype:      'dropdownlist',
        filtertype:      'checkedlist',
        filteritems:     powerfactorsAdapter,
        createeditor:    function (rowIndex, cellValue, editor) {
                           dropdownCreateEditor (rowIndex, cellValue, editor, {
                             dropDownWidth: 50,
                             source: powerfactorsAdapter,
                           });
                         },
        initeditor:      dropdownInitEditor,
        cellbeginedit:   function (rowIndex) {
                           var division = $(grid).jqxGrid ('getrowdata', rowIndex).sh_dvp_fmt;
                           return ((division === 'Carry Optics') || (division === 'Production')) ? false : true;
                         },
      },
      {
        text:            'Class',
      ...
      },
      {
        ...
        validation:      function (cell, value) {
                           ...
                           if (text.length === 0)
                             return ({result: false, message: 'EZWS # cannot be empty (use -1 for no number)'});
                           if ((text.match (/^[-]?[0-9]{1,4}$/) === -1) || ((sh_num !== -1) && ((sh_num < 1) || (sh_num > 9999))))
                             return ({result: false, message: 'EZWS # must be -1, or 1..9999'});
                           if (sh_num !== -1) {
                             var sh_uid = App.matchData.m.match_shooters [cell.row].sh_uid;
                             var count = 0;
                             ...
                             if (count)
                               return ({result: false, message: 'This EZWS # is already in use by another competitor'});
                           }
  ...
  $(grid).on ('cellclick', function (event) {
    if (event.args.rightclick && !$(grid).find ('.jqx-grid-validation').length) {
      App.socket.emit ('shooter:print:inqueue', function (data) {
        ...
        $('#contextEzwsSetNum').text (event.args.rowindex + 1);
  ...
  App.contextMenuInit = function () {
    ...
    _.each (['sh_pos','sh_ln','sh_fn','sh_id','sh_dvp_fmt','sh_pf_fmt','sh_grd','sh_sqd','sh_ctgs_fmt','sh_age_fmt','sh_gen_fmt','sh_law_fmt','sh_mil_fmt','sh_frn_fmt','sh_eml','sh_ph','sh_st','sh_num','sh_wlk_fmt','sh_dq_fmt','sh_del_fmt','sh_print_fmt'], function (n) {
      var sc = $('#sc_' + n);
    ...
    $('#contextStripBadMembershipNumber').each (function () {
      $(this).jqxTooltip ({
        width: 200,
        position: 'default',
        content: 'These options will strip the competitor\'s USPSA number if it is improperly formatted (contains \'NONE\', \'PEN\', phone number, etc)',
        showDelay: 500,
        autoHideDelay: 0
      });
    });
    ...
    $('#confirmResequenceWindow').jqxWindow ({
      height: 185,
      width: 315,
      resizable: false,
      autoOpen: false,
      ...
      okButton: $('#buttonResequenceOK'),
      cancelButton: $('#buttonResequenceCancel'),
      initContent: function () {
        $('#buttonResequenceOK').jqxButton ({width: 65});
        $('#buttonResequenceCancel').jqxButton ({width: 65});
        $('#buttonResequenceCancel').focus();
      },
    });
    ...
    $(contextMenu).on ('itemclick', function (event) {
      ...
      switch (event.args.id) {
        ...
        case 'contextEzwsSet' :
          App.socket.emit ('shooter:ezws:set', {uid: App.matchData.m.match_shooters [rowIndex].sh_uid}, function (data) {
            if (!data.err) {
              App.matchData.m.match_shooters [rowIndex] = data.shooter;
              $(grid).jqxGrid ('updatebounddata', 'cells');
            } else
          ...
          break;
        case 'contextEzwsResequence' :
          App.socket.emit ('shooter:ezws:status', function (data) {
            var reseq = function () {
              App.socket.emit ('shooter:ezws:resequence', function (data) {
                if (!data.err) {
                  shooterSource.localdata = data.matchdef.m.match_shooters;
                  $(grid).jqxGrid ('updatebounddata', 'cells');
                } else
            ...
            if (data.allNegativeOne || data.alreadyInSequence)
              reseq ();
            else {
              $('#confirmResequenceWindow').off ('close');
              $('#confirmResequenceWindow').on ('close', function (event) {
                if ((event.type === 'close') && event.args.dialogResult.OK)
                  reseq ();
              });
              $('#confirmResequenceWindow').jqxWindow ('open');
            }
          });
          break;
        case 'contextEzwsReset' :
          App.socket.emit ('shooter:ezws:reset', function (data) {
            if (!data.err) {
              App.matchData.m = data.matchdef.m;
              shooterSource.localdata = data.matchdef.m.match_shooters;
              $(grid).jqxGrid ('updatebounddata', 'cells');
  ...
  App.popupEditInit = function () {
    ...
    $('#ec-sh-pf').jqxDropDownList ({width: 65, autoDropDownHeight: true, dropDownWidth: 50, source: powerfactorsAdapter});
    ...
    $('#ec-sh-num').jqxInput ({width: 50, height: 25, maxLength: 4});
    ...
    $('#ec-sh-dvp').on ('change', function (event) {
      if ((event.args.item.label === 'Carry Optics') || (event.args.item.label === 'Production')) {
        ...
        $('#ec-sh-pf').jqxDropDownList ('selectItem', editVars.popupDataRecord.sh_pf_fmt);
    ...
    $(popupEdit).jqxValidator ({
      ...
      onSuccess: function () {
        ...
        dataRecord.sh_num     = parseInt ($('#ec-sh-num').val ());
        dataRecord.sh_paid    = $('#ec-sh-paid').jqxDropDownList ('getSelectedItem').value;
        dataRecord.sh_pf      = $('#ec-sh-pf').jqxDropDownList ('getSelectedItem').value;
        ...
        dataRecord.sh_pf_fmt  = $('#ec-sh-pf').jqxDropDownList ('getSelectedItem').label;
      ...
      rules: [
        ...
        },
        {
          input:    '#ec-sh-id-num',
          position: 'bottom',
          message:  'This USPSA # is in use by a competitor with a different name',
          action:   'valuechanged, blur',
          rule:     function (input) {
                      var num = input.val ().trim ();
                      var prefix = $('#ec-sh-id-prefix').val ().trim ();
  ...
  App.socketDisconnect = function () {
    ...
    $('#popupEdit').jqxWindow ('close');
    $('#popupSquads').jqxWindow ('close');
    $('#confirmResequenceWindow').jqxWindow ('close');
  ...
  App.socket.emit ('log:log', {msg: 'Edit/View->Competitors (USPSA)'});
