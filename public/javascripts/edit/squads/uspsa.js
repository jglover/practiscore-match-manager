...
$(function () {
  ...
  App.addMember = function (shooterIndex, shooter) {
    ...
    if (shooter.sh_del)
      ...
      li.style.cssText = 'background-color: lightgreen;';
    else if (shooter.sh_isro)
      li.style.cssText = 'background-color: lightblue;';
    ...
    var sortableOptions = {
      Left: {
        ...
        receive: function (event, ui) {
          ...
          App.socket.emit ('shooter:squad', {
            uid: ui.item.attr ('uid'),
            squad: $(this).attr ('squadnumber'),
          });
  ...
  //
  App.reload = function () {
    window.location.href = 'http://' + window.location.host + '/edit/squads';
  };
  ...
  App.socket.emit ('log:log', {'msg': 'View/Edit->Squads (USPSA)'});
});
