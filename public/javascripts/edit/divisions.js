...
$(function () {
  ...
  var divisionsSource = {
    ...
    addrow: function (rowid, newdata, position, commit) {
      ...
      App.socket.emit ('divisions:add', {division: temp}, function (data) {
        if (data.err) {
          ...
          commit (false);
          divisionsSource.localdata = data.divisions;
          $(grid).jqxGrid ({source: divisionsAdapter});
        }
      });
    },
    updaterow: function (rowid, newdata, commit) {
      var temp = [];
      temp.push (newdata);
      App.socket.emit ('divisions:modify', {division: temp}, function (data) {
        if (data.err) {
          editVars.errorDisplayed = true;
          _.each (data.err, function (e) {
            $(grid).jqxGrid ('showvalidationpopup', rowid, e.field, e.errmsg);
    ...
    deleterow: function (rowid, commit) {
      ...
      App.socket.emit ('divisions:delete', {division: temp}, function (data) {
        if (data.err)
          commit (false);
        else {
          commit (false);
  ...
  divisionsAdapter = new $.jqx.dataAdapter (divisionsSource);
  ...
  var openPopupEditorAdd = function () {
    ...
    $(popupEdit).jqxWindow ('title', 'Add Division');
  ...
  var openPopupEditorEdit = function (rowIndex) {
    ...
    $(popupEdit).jqxWindow ('title', 'Editing Division #' + (rowIndex + 1));
    $(popupEdit).jqxWindow ('open');
  };
  ...
  var gridColumns = [
      {
        ...
        validation:      function (cell, value) {
                           if (!value.length)
                             return ({result: false, message: 'PMM division name is required'});
                           if (value.search (/[^A-Za-z0-9 ]/) !== -1)
                             return ({result: false, message: 'PMM division name may only contain the characters A-Z, a-z, 0-9, and space'});
                           return true;
                         },
      }, {
        ...
        validation:      function (cell, value) {
                           if (!value.length)
                             return ({result: false, message: 'PractiScore division name is required'});
                           if (value.search (/[^A-Z0-9]/) !== -1)
                             return ({result: false, message: 'PractiScore division name may only contain the characters A-Z and 0-9'});
                           return true;
                         },
      }, {
        text:            'Enabled',
  ...
  $(grid).jqxGrid ({
    ...
    rendered: function () {
      ...
      gridCells.on ('dragEnd', function (event) {
        ...
        if (!_.isNaN (dropRow)) {
          App.socket.emit ('divisions:move', {from: dragRowData, to: dropRowData}, function (data) {
            if (data.changed) {
              divisionsSource.localdata = data.divisions;
              $(grid).jqxGrid ({source: divisionsAdapter});
            }
  ...
  $(grid).on ('cellclick', function (event) {
    if (event.args.rightclick && !$(grid).find ('.jqx-grid-validation').length) {
      ...
      $('#contextEditDivisionNum').text (rowindex + 1);
      $('#contextAddDivisionNum').text ($(grid).jqxGrid ('getdatainformation').rowscount + 1);
      $('#contextDeleteDivisionNum').text (rowindex + 1);
      var scrollTop = $(window).scrollTop ();
      var scrollLeft = $(window).scrollLeft ();
      var contextMenu = App.contextMenuInit ();
      $(contextMenu).jqxMenu ('open', parseInt (event.args.originalEvent.clientX) + 5 + scrollLeft, parseInt (event.args.originalEvent.clientY) + 5 + scrollTop);
      $(contextMenu).jqxMenu ('disable', 'contextEditDivision', readonly);
      $(contextMenu).jqxMenu ('disable', 'contextDeleteDivision', readonly);
      return false;
    }
  });
  ...
  App.contextMenuInit = function () {
    ...
    $(contextMenu).on ('itemclick', function (event) {
      ...
      switch (event.args.id) {
        case 'contextEditDivision' :
          openPopupEditorEdit (selectedrowindex);
          break;

        case 'contextAddDivision' :
          openPopupEditorAdd ();
          break;

        case 'contextDeleteDivision' :
      ...
      rules: [
        {
          input:    '#ec-pmmname',
          position: 'bottom',
          message:  'PMM division name is required',
          action:   'keyup, blur',
          rule:     'required'
        }, {
          input:    '#ec-pmmname',
          position: 'bottom',
          message:  'PMM division name may only contain the characters A-Z, a-z, 0-9, and space',
          action:   'keyup, blur',
          rule:     function (input) {
          ...
          input:    '#ec-psname',
          position: 'bottom',
          message:  'PractiScore division name is required',
          action:   'keyup, blur',
          rule:     'required'
        }, {
          input:    '#ec-psname',
          position: 'bottom',
          message:  'PractiScore division name may only contain the characters A-Z and 0-9',
          action:   'keyup, blur',
          rule:     function (input) {
  ...
  App.refreshMatchData = function () {
    App.socket.emit ('divisions:get', function (data) {
      divisionsSource.localdata = [];
      _.each (data.divisions, function (division) {
        divisionsSource.localdata.push ({
          ps_name:  division.ps_name,
          pmm_name: division.pmm_name,
